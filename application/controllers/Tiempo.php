...
class Tiempo extends MY_Controller {
	private $request;
    ...
    public function index() {
        ...
        $this->load->view('menu');
        $this->load->view('Tiempos/index');
        $this->load->view('footer',["js"=>[""]]);
      }
			public function gettiempos($id=0) {

				  $data['tiempos'] = $this->tiempo->gettiempos();
				  header('Content-Type: application/json');
				  echo json_encode(['tiempos' => $data['tiempos']]);
			...
			public function insertar() {
				...
				$result = $this->tiempo->insertar($data);
					if($result['code'] == 0){
			...
			public function editar() {
			    ...
			    $data = json_decode($this->input->post('service_form'),true);
				$result = $this->tiempo->editar($data);
					if($result['code'] == 0){
			...
			public function eliminar() {
	            ...
	            $result = $this->tiempo->deletetiempo($id);
	               if($result['code'] == 0){
